...
7. **Verify the exact R library contract.** Require R 4.5.3 and `/mnt/disk1/0714_control/r_libs/zCompositions-1.6.2-R-4.5.3` as the first effective `.libPaths()` entry. Require zCompositions 1.6.2, NADA and truncnorm to resolve inside that directory; record all three versions and paths. A matching package from any other library fails. Ensure the system package set/versions are unchanged.
