...
class Lifecycle(unittest.TestCase):
 ...
 def test_real_queue_checkout_done_again(self):
  with tempfile.TemporaryDirectory() as t:
   ...
   self.sh('git','checkout','-b','etty-handoff',cwd=seed); self.sh('git','push','origin','etty-handoff',cwd=seed)
   q=d/'queue'; q.mkdir(); (q/'j.json').write_text(json.dumps(env)); qr=d/'qr'; jr=d/'jr'; hr=d/'hr'; self.sh('git','clone',str(bare),str(qr)); self.sh('git','clone',str(bare),str(jr)); self.sh('git','clone',str(bare),str(hr)); self.sh('git','checkout','-B','etty-handoff','origin/etty-handoff',cwd=hr); state=d/'state.json';
   ...
   self.assertEqual(process(env,qr,jr,hr,state),'DONE'); self.assertEqual(process(env,qr,jr,hr,state),'ALREADY_COMPLETED'); self.assertEqual(self.sh('git','rev-parse','HEAD',cwd=jr),commit)
